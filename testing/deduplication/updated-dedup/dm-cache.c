...
#define DEDUP 			0
struct bucket * fingerprint_table;
...
struct cacheblock {
	...
	//Dedup
	int ref_cnt;		/* Number of referencing sources */
	struct list_head refs;  /* List of referencing sources */
...
static int clear_old_refs(struct cache_c * dmc, struct cacheblock * cache)
{
	struct ref * post;
	struct ref * n;
	struct ref * tmp;
	...
	list_for_each_entry_safe(post, n, &cache->refs, list)
	{
		tmp = list_entry(&post->list, struct ref, list);
		cache->ref_cnt--;
		list_del(&post->list);
		radix_tree_delete(dmc->cache, tmp->radix);
	}
...
static int do_store(struct kcached_job *job)
{
	...
	if(DEDUP)
	{
		...
		fingerprint = kmalloc(sizeof(char) * 16, GFP_KERNEL);
		make_fingerprint(job->bio, fingerprint, dmc->block_size);
		store = find_fingerprint(fingerprint_table, fingerprint, dmc->size);

		if(!store)
		{
			if(job->cacheblock->ref_cnt >= 1)
			{
				clear_old_refs(dmc, job->cacheblock);
				remove_fingerprint(fingerprint_table, fingerprint, dmc->size);
				kfree(job->cacheblock->fingerprint);
			}
			...
			if (bio_data_dir(bio) == READ)
			{
				ref = create_ref(radix, src_idx, disk, 0);
			}
			else
			{
				ref = create_ref(radix, src_idx, disk, 1);
			}
		...
		{
			radix_tree_delete(dmc->cache, radix);
			radix_tree_insert(dmc->cache, radix, (void *) &dmc->blocks[store->cacheblock]);
			...
			else
			{
				...
				if (is_state(dmc->blocks[store->cacheblock].state, VALID))
				{
					set_state(dmc->blocks[store->cacheblock].state, DIRTY);
					dmc->dirty_blocks++;
					virtual_mapping[dmc->blocks[store->cacheblock].disk].dirty_blocks++;
				}
			}

			dmc->blocks[store->cacheblock].ref_cnt += add_ref(&dmc->blocks[store->cacheblock].refs, ref);

			if(job->cacheblock->ref_cnt >= 1)
			{
				spin_lock(&job->cacheblock->lock);
        			job->cacheblock->state = RESERVED;
				spin_unlock(&job->cacheblock->lock);
			}
			else
			{
				spin_lock(&job->cacheblock->lock);
        			job->cacheblock->state = WRITETHROUGH;
				spin_unlock(&job->cacheblock->lock);
			}
			...
			if(DEDUP)
			{
				if(bio_rw(bio) == WRITE)
				{
					if(cache->ref_cnt > 1)
					{
						...
						{
							cache->ref_cnt--;
							cache->ref_cnt += remove_ref(&cache->refs, get_block_index(request_block,disk));
						...
						else
						{
							...
							if(store == NULL || store->cacheblock != cache->cacheblock)
							{
								cache->ref_cnt--;
								cache->ref_cnt += remove_ref(&cache->refs, get_block_index(request_block,disk));
								return cache_miss(dmc, bio, disk);
...
static int cache_ctr(struct dm_target *ti, unsigned int argc, char **argv)
{
	...
	DMINFO("Allocate %lluKB (%luB per) mem for %llu-entry cache" \
		...
		if(DEDUP)
		{
			dmc->blocks[i].ref_cnt = 0;
			INIT_LIST_HEAD(&dmc->blocks[i].refs);
			dmc->blocks[i].fingerprint = NULL;
		}
        }
